...
<?
	...
	foreach ($existing_featured_list as $existing_featured_entry)
	{
		...
		{
			$status = 'Pending';
			$cancel_link = $html->link("Cancel", "/admin/member_featured_models/cancel/$featured_model_id/$member_id");
		} else {
			# Start time < now time
...
<a href="/admin/member_featured_models/checkout/<?= $member_id ?>">Signup Now</a>! (PayPal Required)
