		...
		<?
			...
			$about_me = preg_replace("/(\w{32})/", "\\1 ", $member['MemberDesignerProfile']['about_me']);

			if ($url = $member['MemberDesignerProfile']['website']) {
			...
			echo $html->tableCells(array(
				array(
					...
					"<label>Gender:</label>".$member['MemberDesignerProfile']['gender']."&nbsp;",
				),
				array(array(
					"<label>Employer Name:</label>". $member['MemberDesignerProfile']['employer_name'] . "&nbsp;",
					array('colspan'=>2))),
				array(
